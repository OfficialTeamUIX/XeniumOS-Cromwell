...
unsigned char accepted_xremote [] = { 0x0b, 0xa6, 0xa7, 0xa9, 0xa8 ,0xd5,0xd8,
				     0xc6,0xc7,0xc8,0xc9,0xca,0xcb,0xcc,0xcd,0xce,0xcf,0xf7};
	...
	// Section for IR Driver   
       ...
       if ((temp&0x100)==0x100) {
		...
		if  ( 1==1 ){	
			...
			switch (selected_Button) {
				case TRIGGER_XPAD_KEY_A :
			   				   if ((current_remote_key ==  0xb) || (current_remote_key == 0xce)) returnvalue = 1; 
							   break;
				case TRIGGER_XPAD_KEY_B :
			   				   if ((current_remote_key ==  0xd8) || (current_remote_key == 0xcd)) returnvalue = 1; 
							   break;				
				case TRIGGER_XPAD_KEY_D:
			   				   if (current_remote_key ==  0xcb) returnvalue = 1; 
							   break;
				case TRIGGER_XPAD_KEY_C:
			   				   if (current_remote_key ==  0xcc) returnvalue = 1; 
							   break;
				case TRIGGER_XPAD_KEY_BACK:
			   				   if (current_remote_key ==  0xf7) returnvalue = 1; 
							   break;
				case TRIGGER_XPAD_KEY_START:
			   				   if (current_remote_key ==  0xcf) returnvalue = 1; 
							   break;
				case TRIGGER_XPAD_PAD_UP :
...
//        	Button = XPAD_current[0].keys[selected_Button];
        	Button = XPAD_gCurrent.keys[selected_Button];
	        if (Button==0x0)
	        {
	              	Button_actual = 0;	
	        }
 	...
 	if ((selected_Button > 5) & (selected_Button < 10) ) {
		...
		// Rising Edge
		if (((XPAD_gCurrent.pad&Buttonmask) != 0) & ((xpad_button_history[6]&Buttonmask) == 0)) {
